//
// $Id: LogErrorAnalysis.cc,v 1.3 2011/02/04 23:43:53 venturia Exp $
//
...
/**
  ...
  \version  $Id: LogErrorAnalysis.cc,v 1.3 2011/02/04 23:43:53 venturia Exp $
*/
...
class LogErrorAnalysis : public edm::EDProducer {
    public:
        explicit LogErrorAnalysis(const edm::ParameterSet & iConfig);
        virtual ~LogErrorAnalysis() { }

        virtual void produce(edm::Event & iEvent, const edm::EventSetup & iSetup);
        virtual void beginLuminosityBlock(edm::LuminosityBlock &lumi, const edm::EventSetup &iSetup);
        virtual void endLuminosityBlock(edm::LuminosityBlock &lumi, const edm::EventSetup &iSetup);
...
void
LogErrorAnalysis::produce(edm::Event & iEvent, const edm::EventSetup & iSetup) {
  bool fail = false, save = false;
  std::auto_ptr<bool> pOut( new bool(save) ); 
    ...
    if (errors->empty()) { 
        npassRun_++; npassLumi_++;
        save = false;
        iEvent.put( pOut, "TrackerLogError" );
        return;
    } 
